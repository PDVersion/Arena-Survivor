...
describe("expanded enemy roster", () => {
  it("uses the product baselines for Runner and Tank", () => {
    ...
    expect(byId.get(archetypeIds.enemy.fastFragile)).toMatchObject({ maxHealth: 10, moveSpeed: 140, contactDamage: 8, xpReward: 1 });
    expect(byId.get(archetypeIds.enemy.slowDurable)).toMatchObject({ maxHealth: 80, moveSpeed: 45, contactDamage: 20, xpReward: 4 });
  });
