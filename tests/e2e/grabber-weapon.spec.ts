...
test("the cleanup grabber damages every enemy in its path without creating a projectile", async ({ page }) => {
  await page.goto("/?loadHarness=3&closeLoad=1&lineLoad=1&noContact=1&noXp=1");
  await expect
  ...
  expect(snapshot?.combat?.meleeHits).toBeGreaterThan(snapshot?.combat?.meleeStrikes ?? 0);
});
